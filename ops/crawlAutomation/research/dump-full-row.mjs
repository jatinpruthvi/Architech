...
// Fetch 1 record via in-browser fetch+decrypt and dump full raw JSON
const row = await page.evaluate(async () => {
  const body = new URLSearchParams({
    draw: 1, start: 0, length: 1, searchvalue: 'all',
  ...
  const r = dec.data[0];
  // dump every column (0-19) fully
  const out = [];
  for (let i = 0; i < r.length; i++) {
    const val = r[i];
    const str = typeof val === 'string' ? val : JSON.stringify(val);
    // also note if contains image/gallery/html
    const hasGallery = typeof val === 'string' && (val.includes('image-gallery') || val.includes('<img ') || val.includes('src='));
    out.push({ col: i, len: str.length, hasGallery, preview: str.slice(0, 400) });
  }
  return out;
});

console.log('=== FULL ROW COLUMNS ===');
for (const c of row) {
  console.log(`\n--- col ${c.col}  len=${c.len}  hasGallery=${c.hasGallery} ---`);
  console.log(c.preview);
}

writeFileSync('tp-full-row.json', JSON.stringify(row, null, 2));
await context.close();