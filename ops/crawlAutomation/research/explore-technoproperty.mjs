...
// Get the dashboard page and save it for analysis
const dash = await curl('dashboard', 'https://ahmedabad.technoproperty.in/dashboard.php');
writeFileSync('tp-dashboard.html', dash.body);
console.log('Dashboard saved:', dash.size, 'bytes');

// Find all links in dashboard
const links = [...new Set(dash.body.match(/href="([^"]+)"/g) || [])].map(m => m.match(/href="([^"]+)"/)[1]);
console.log('\n=== All links in dashboard ===');
links.forEach(l => console.log('  ', l));

// Find all .php files referenced
const phps = [...new Set(dash.body.match(/[\w\-]+\.php/g) || [])];
console.log('\n=== PHP files referenced ===');
phps.forEach(p => console.log('  ', p));

// Find any AJAX/API endpoints
const ajax = [...new Set(dash.body.match(/(?:url|action|href|src|fetch|ajax)[^"']*["']([^"']+\.php[^"']*)/g) || [])];
console.log('\n=== AJAX/API endpoints ===');
ajax.forEach(a => console.log('  ', a));
