export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    /* The SEO event spine's consumers are wired here, once, at startup.
       ...
       by convention. */
    const { registerSeoDiscovery } = await import("./src/lib/seo/discovery");
    ...
    const { registerSavedSearchAlertRuntime } = await import("./src/lib/saved-search/alerts-runtime");
    ...
    const { registerLeadNotificationRuntime } = await import("./src/lib/leads/notifications-runtime");
    ...
    const { registerMediaRetentionRuntime } = await import("./src/lib/media/retention-runtime");
