...
The fix introduces a shared `.field-shell` contract: controls inside the shell draw no outline of their own, and the shell paints a single 2px focus edge at `outline-offset: 0` so it hugs the existing border instead of reading as a second one. The colour comes from `--field-focus` (ember on the dark hero, brick elsewhere), and a `@supports not selector(:has(input))` fallback keeps one visible edge on engines without `:has()`. The hero composer's redundant `focus-within:ring-1` was removed, and the results search and agent workspace filter now use the same shell. A regression test in `src/lib/ui/field-focus.test.ts` locks the contract in place.
