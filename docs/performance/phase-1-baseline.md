...
**Sixth re-baseline (26 Aug 2026, locality-intelligence evidence band):** the `/buy/ahmedabad/:locality/` hub gained a provenance-labeled evidence layer (current price range with "Based on N active verified buy listings · Updated <date>", inventory by configuration & budget, commute/nearby essentials, and new projects), replacing fabricated placeholder snapshot figures (`homes*3`, `₹11.2k`, `4.8 km`, `92%`) that conflicted with the "do not present inferred data as verified facts" trust rule. This is a cross-route client component (bundled into a shared chunk that `/search` already loaded), adding ~22.3 KiB total static JS. Because the feature is a genuine product need (principal differentiator from the 2026 product direction) and corrects a data-provenance issue, the budget was re-baselined rather than the feature stripped: first-load gzip cap raised 235,000→240,000 B (measured `/search` 230.0 KiB → ≈9.3 KiB headroom) and total-static-JS cap raised 2,190,000→2,212,000 B (measured 2151.1 KiB → ≈9.1 KiB headroom). Per-route raw first-load, HTML, image, and Core Web Vitals targets are unchanged and still strict.

**No-cap change (5 Sep 2026, audit fixes — see `performance-audit-2026-09-05.md`):** no budget moved. (a) `/search` first-load gzip was measured at 245,153 B — 153 B *over* the 245,000 B cap after the R2 render-path commit added +358 B onto 205 B of headroom. Fixed by code splitting: the filter sheet (the only static `vaul` consumer) became a dynamic chunk (`FilterSheet.tsx`), dropping `/search` first-load to 233,914 B gzip (−11.2 KiB, ≈11 KiB headroom restored). (b) The render-blocking Google Fonts link was replaced with self-hosted Fontsource woff2 (unicode-range subsets, `font-display: swap`) — font CSS now rides in the same-origin stylesheet, font files fetch in parallel from the same origin, and CSP dropped the Google origins. All other budgets unchanged and passing.
