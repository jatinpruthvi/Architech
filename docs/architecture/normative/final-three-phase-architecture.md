...
This section is normative and was added after the architecture governance review. It resolves contradictions and connects the architecture to executable implementation work.

## Normative supporting documents

The implementation source of truth is supported by:
...
DECISION-LOG.md
LEGAL-GATES.md
SUPERSESSION-MANIFEST.md
```

Historical recommendations and feedback reports remain available for context but are not independent specifications.

## Capability readiness model

A capability moves through the following states:

```text
planned → contracted → implemented → validated → enabled → retired
```

Phase 1 establishes the approved capability contract and foundation. A capability may remain disabled after implementation until performance, cost, security, editorial, legal, accessibility, or operational gates pass. This distinction preserves the complete product scope without exposing unvalidated behavior.

## Resolved lead policy

Masked contact is the default public behavior. A user can explicitly choose a consented direct-contact mode only when the broker is eligible, the UI clearly explains the mode, consent is recorded, rate limits and abuse controls pass, and the transition is auditable. Lead state transitions, retries, dead-letter behavior, deletion, retention, and notification rules are defined in `DOMAIN-CONTRACTS.md`.

## Resolved infrastructure policy

Railway is the primary persistent-services platform for production API, workers, jobs, private services, and operational environments. Containers remain portable. Fly.io may be used as a future migration or recovery target, but it is not a co-equal Phase 1 production platform.

## Measurable target policy

The architecture uses measurable hypotheses instead of guarantees. It must not promise rankings, unbeatable performance, fixed competitor outcomes, universal Core Web Vitals results, or guaranteed AI citation. Every target requires a route class, device/network profile, data source, owner, review date, and reversal trigger.

## Legal release policy

RERA ingestion, public-record republication, personal-data processing, broker media, email, sponsored links, AI-generated claims, localization, and commercial lead workflows require the corresponding approval in `LEGAL-GATES.md` before public enablement.

## Implementation traceability

Every active work item must link:
...
A failed acceptance test creates a remediation task and does not silently weaken the requirement. Changes to this architecture must update the decision log and supersession manifest.
