...
```text
src/components/architech/TrustPanel.tsx
...
- **SEO regression net.** `ops/scripts/seo/raw-html-smoke.mjs` now asserts the listing page contains `trustScore`, `additionalProperty`, `Trust score`, and the `RERA verified` badge.
...
node ops/scripts/performance/budget.mjs
```
