...
- MCP path: the local/remote server handshake was tested this date; tools available: `search_prompts`, `get_prompt` (server `prompts-chat` v1.0.9).
...
- Note: sandbox network could not reach prompts.chat over TLS directly; community validation used the tool-side fetcher against the canonical dataset instead.
