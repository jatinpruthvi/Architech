...
**Repository revision reviewed:** `a77e526` — `feat(ops/config/governance/order): authority registry + API, media-attach audit hardening (#30)`
...
The following checks were run against the pulled repository. `pnpm check` passed. `pnpm lint` completed with one warning for an unused `authorityOutreachGet` import in [`src/lib/api-contract.test.ts`](../src/lib/api-contract.test.ts). The unit suite ran 47 test files: 46 passed and one failed. The failing test is [`src/lib/auth/auth.test.ts`](../src/lib/auth/auth.test.ts), where the test expects `DATABASE_URL` to be reported missing, but the process environment already supplies it. The implementation in [`src/lib/auth/source.ts`](../src/lib/auth/source.ts) is therefore being tested against ambient environment state rather than a hermetic fixture.
...
This should cover pages, structured data, sitemaps, OG images, guides, locality counts, broker identity, and RERA evidence. It should also prevent illustrative values such as `GJ/RERA/AHM/2026/04821-DEMO` in [`db/seed.mjs`](../db/seed.mjs) from entering a production sitemap or public trust surface.
...
The README simultaneously says that the repository is “the architecture source, not the application source code” and that it contains the working Next.js application ([`README.md`](../README.md), lines 7–11). That contradiction will mislead both human engineers and AI coding systems. Replace it with a clear split: the repository is the **architecture, governance, and reference implementation**; the `app/`, `client/`, `db/`, `seo/`, and `ops/config/governance/` directories are named sources of truth for their respective concerns.
...
[4]: ../src/lib/auth/source.ts "Better Auth environment contract"  
[5]: ../src/lib/auth/auth.test.ts "Better Auth contract tests"  
...
[10]: ../db/seed.mjs "Illustrative seed data and demo RERA fixture"  
[11]: ../IMPROVEMENT-REVIEW-2.md "Historical Vite-era improvement review"  
[12]: ../src/lib/api-contract.test.ts "API contract test with lint warning"
