...
5. Evolution events write full message bodies, media, and status markers to the business's own Frappe site (single-tenant DB). The employee can read and reply to these messages directly from the WhatsApp tab in Frappe CRM, or from their native company WhatsApp app (which syncs bidirectionally via the Evolution connection).
