...
**Date:** 08 Sep 2026
**Status:** Proposed — awaiting decisions in §4
...
These change scope materially and are not mine to assume.
...
export type LeadContact = {
  ...
  waMeLink?: string;
  revealed: boolean;
  blockedReason?: "NO_PERMISSION" | "CONSENT_CLASS" | "SUPPRESSED"
                | "OUTSIDE_HOURS" | "ATTEMPT_LIMIT" | "NOT_STORED";
};
```
...
- [ ] Add a `package.json` entry alongside `audit:contrast`
- [ ] Add overflow + tap-target assertions to `tests/a11y-broker` under the existing `chromium-mobile` / Pixel 5 project (M5)
- [ ] Record the baseline table in this doc so regressions are diffs, not opinions

### Phase 1 — Mobile shell fixes, no data change (1–2 days)
- [ ] M1: server-render broker page structure + skeleton (all 6 routes)
- [ ] M3: `PropertyCard` grid mobile-first
- [ ] M4: `/broker/leads/[id]/` detail route rendering existing masked data
- [ ] Sticky bottom action bar with **WhatsApp/copy** only (no call yet)
- [ ] Pay down `BrokerLeadInbox` token debt while rewriting it
