...
node ops/scripts/seo/crawl-simulation.mjs   # 492 pages crawled, 444 sitemap URLs, 0 problems (fixture + indexing enabled)
...
|---|---|---|---|
| D5-01 | No layout-default canonicals; noindex utility pages self-reference. | A layout canonical stampede once leaked the homepage canonical onto four unrelated pages. | `ops/scripts/seo/crawl-simulation.mjs` canonical checks (491 pages) |
...
| D5-10 | Entity locale completeness is a test: every fixture city/locality AND seed city/locality carries a Hindi name containing Devanagari (contains-check, not exclusivity — `साल्ट लेक सेक्टर V` keeps its official Roman numeral), and fixture↔seed city parity holds. | `row.hindiName ?? row.name` fallback makes missing Hindi names invisible until someone reads a Hindi page aloud. | `src/lib/entity-locale.test.ts` (5 checks) |
...
- `ops/scripts/seo/crawl-simulation.mjs` — segments, canonical-origin link resolution, indexed-surface boot; now a CI step.
- `ops/scripts/seo/run-seo-suite.mjs` + `package.json test:seo` — fixture-pinned SEO suite.
...
- `src/lib/entity-locale.test.ts` — locale-field contract.
- `src/lib/repositories/mappers.ts` — `meaningfulUpdatedAt` survives to the page (D5-07).
- `src/lib/auth/guards.ts` — documented demo-in-production escape hatch (D5-05).
- `src/components/architech/CommandPalette*.tsx` — palette on semantic tokens.
- `src/components/architech/Footer.tsx` — City price index link (D5-03).
