...
const navItems = [
  { href: "/buy/mumbai/", label: "Explore Mumbai" },
...
function SiteHeader() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? "bg-paper/95 shadow-[0_8px_30px_rgba(39,34,28,.08)] backdrop-blur-xl" : "bg-paper/75 backdrop-blur-md"}`}>
      <div className="container flex h-[76px] items-center justify-between">
        <Link href="/" className="group flex items-center gap-3" aria-label="Architech home">
          <span className="grid h-10 w-10 place-items-center bg-ink p-2 transition-transform duration-200 group-hover:-rotate-3">
            <img src="/manus-storage/architech-mark_56302218.png" alt="" className="h-full w-full object-contain" />
          </span>
          <span className="font-display text-[21px] font-semibold tracking-[-.04em] text-ink">architech<span className="text-clay">.</span></span>
        </Link>
        <nav className="hidden items-center gap-8 md:flex" aria-label="Primary navigation">
          {navItems.map((item) => <Link key={item.href} href={item.href} className="text-[13px] font-medium tracking-[.03em] text-ink/70 transition-colors hover:text-clay">{item.label}</Link>)}
        </nav>
        <div className="hidden items-center gap-4 md:flex">
          <Link href="/saved" className="inline-flex items-center gap-2 text-[13px] font-medium text-ink/70 hover:text-clay"><Bookmark size={15} strokeWidth={1.7} /> Saved</Link>
        ...
        <button className="grid h-10 w-10 place-items-center text-ink md:hidden" onClick={() => setOpen(!open)} aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open}>{open ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
  ...
  return <>
    ...
    <main>
      <Switch>
        ...
        <Route path="/buy/mumbai/" component={CityPage} />
    ...
    <footer className="border-t border-ink/10 bg-paper py-10">
      <div className="container flex flex-col gap-5 text-sm text-ink/60 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-5"><Link href="/guide" className="hover:text-clay">Methodology</Link><Link href="/buy/mumbai/" className="hover:text-clay">Mumbai homes</Link><Link href="/search" className="hover:text-clay">Search</Link></div>
      </div>
    </footer>
  </>;
}
