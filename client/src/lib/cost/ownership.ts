...
export type OwnershipAssumptions = {
  ...
  /** Stamp duty rate as a fraction (e.g. 0.05 = 5%). */
  stampDutyRate?: number;
  /** Registration fee fraction (default 0.01). */
...
export const DEFAULT_STAMP_DUTY = 0.05;
export const DEFAULT_REGISTRATION = 0.01;
...
export function calculateOwnershipCost(input: OwnershipAssumptions): OwnershipCost {
  ...
  const stampDutyRate = input.stampDutyRate ?? DEFAULT_STAMP_DUTY;
  const registrationRate = input.registrationRate ?? DEFAULT_REGISTRATION;
