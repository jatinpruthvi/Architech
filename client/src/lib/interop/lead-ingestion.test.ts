...
/* A MagicBricks-shaped push payload (their documented field names), mapped by
   the adapter into the contract. */
function magicbricksLead(overrides: Partial<IngestedLeadInput> = {}): IngestedLeadInput {
  return {
    source: "magicbricks",
    ...
    consent: {
      ...
      evidence: "magicbricks:push:MB-59679995-8821",
    },
    ...overrides,
  };
}
...
describe("lead ingestion contract", () => {
  it("normalizes a portal lead end to end", () => {
    const result = validateIngestedLead(magicbricksLead());
  ...
  it("collapses every phone shape onto one dedupe key", () => {
    ...
    const keys = new Set(
      shapes.map((mobile) => {
        const result = validateIngestedLead(magicbricksLead({ mobile }));
  ...
  it("gives first-party form leads automation permissions, portal leads only human touch", () => {
    ...
    const portal = validateIngestedLead(magicbricksLead());
  ...
  it("rejects unknown sources and unknown consent classes instead of guessing", () => {
    const unknownSource = validateIngestedLead(magicbricksLead({ source: "sms-blast" as never }));
    ...
    const unknownConsent = validateIngestedLead(
      magicbricksLead({ consent: { consentClass: "bought-list" as never, capturedAt: "2026-09-08T10:14:00Z", evidence: "x" } }),
    );
  ...
  it("requires a reachable Indian mobile", () => {
    const landline = validateIngestedLead(magicbricksLead({ mobile: "02226257777" }));
  ...
  it("requires consent provenance -- an adapter cannot emit a lead without it", () => {
    const noConsent = validateIngestedLead(magicbricksLead({ consent: undefined as never }));
    ...
    const noEvidence = validateIngestedLead(
      magicbricksLead({ consent: { consentClass: "portal-shared", capturedAt: "2026-09-08T10:14:00Z", evidence: "  " } }),
    );
  ...
  it("rejects inverted and out-of-bounds budgets", () => {
    const inverted = validateIngestedLead(magicbricksLead({ budgetMinInr: 11_000_000, budgetMaxInr: 10_000_000 }));
    expect(inverted.ok).toBe(false);

    const absurd = validateIngestedLead(magicbricksLead({ budgetMinInr: 1e15 }));
    expect(absurd.ok).toBe(false);
  });

  it("requires a name and bounds every string to Frappe's varchar(140)", () => {
    const anonymous = validateIngestedLead(magicbricksLead({ name: {} }));
    expect(anonymous.ok).toBe(false);
    if (!anonymous.ok) expect(anonymous.errors.some((e) => e.includes("first or full name"))).toBe(true);

    const overlong = validateIngestedLead(magicbricksLead({ remarks: "x".repeat(141) }));
  ...
  it("builds bounded, stable idempotency keys per source lead", () => {
    const a = ingestIdempotencyKey("magicbricks", "MB-59679995-8821");
    const b = ingestIdempotencyKey("magicbricks", "MB-59679995-8821");
    const c = ingestIdempotencyKey("99acres", "MB-59679995-8821");
    expect(a).toBe(b);
    expect(a).not.toBe(c);
    expect(a.length).toBeLessThanOrEqual(128);
    expect(a.startsWith(`lead.ingest.magicbricks.v${LEAD_INGESTION_CONTRACT_VERSION}.`)).toBe(true);

    /* A huge provider id must hash, not truncate -- truncation would collide
       two different leads onto one CRM record. */
    const huge = ingestIdempotencyKey("housing", "9".repeat(400));
