  ...
  const toggle = useCallback((id: string) => {
    setCompared((prev) => {
      ...
      if (prev.length >= MAX_COMPARE) {
        lazyToast("Compare holds four homes", { description: "Remove one from the tray below to add another." });
        return prev;
      }
      lazyToast(prev.length === 0 ? "Added to compare" : `Added home ${prev.length + 1} of ${MAX_COMPARE}`, {
