...
function LeadDialog({ propertyTitle }: { propertyTitle: string }) {
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setOpen(false);
    toast("Query sent to Nivasa Partners", { description: "They usually reply within 4 working hours. Your number stays masked. (Demo)" });
  };
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="btn-sweep motion-press mt-8 flex w-full items-center justify-center gap-2 bg-brick px-6 py-5 stamp !text-[12px] font-semibold text-cream"><MessageCircle size={16} /> Ask about this home</button>
      </DialogTrigger>
      <DialogContent className="rounded-none border-ink/15 bg-paper sm:max-w-[440px]">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl font-medium tracking-[-0.02em]">Ask about this home</DialogTitle>
          <DialogDescription className="text-sm leading-6 text-ink/60">
            Your message goes to the verified partner. Your phone number stays <strong className="text-ink">masked</strong> until you choose to share it.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="mt-2 space-y-4">
          <div>
            <label htmlFor="lead-name" className="stamp !text-[10px] text-ink/60">Your name</label>
          ...
          <div>
            <label htmlFor="lead-phone" className="stamp !text-[10px] text-ink/60">Phone (kept masked)</label>
          ...
          <div>
            <label htmlFor="lead-msg" className="stamp !text-[10px] text-ink/60">Message</label>
          ...
          <button type="submit" className="btn-sweep touch-44 w-full bg-night py-4 stamp !text-[12px] font-semibold text-cream">Send query</button>
          <p className="stamp text-center !text-[9px] text-ink/60">Concept preview — no real message is sent.</p>
        </form>
      </DialogContent>
    </Dialog>
  );
...
export default function ListingPage({ id }: { id: string }) {
  ...
  const onSave = () => {
    ...
    toast(nowSaved ? "Saved to your shortlist" : "Removed from shortlist", { description: nowSaved ? `${property.title} · ${property.price}` : undefined });
  };
      ...
      <section className="container py-10 md:py-14">
        ...
        <div className="mt-8 grid gap-6 border-y border-ink/15 bg-sand/70 p-6 md:grid-cols-[1fr_auto] md:items-center md:p-9">
            ...
            <p className="flex flex-wrap items-center gap-2 stamp !text-[11px] font-semibold text-trust">
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-[270px] border border-ink/15 bg-night text-cream">
                  <p className="stamp !text-[10px] text-ember">GJ/RERA/AHM/2026/04821 (sample)</p>
                  <p className="mt-1.5 text-xs leading-5">In production this is checked against the Gujarat RERA registry: registration, promoter, and completion status. Illustrative trail for the concept preview.</p>
                </TooltipContent>
              </Tooltip>
          ...
          <div className="border-l-4 border-brick pl-6 md:min-w-[210px]">
            <p className="stamp !text-[10px] text-ink/60">Guide price</p>
          ...
          <div className="grid gap-4 md:grid-rows-2">
            ...
            <Reveal delay={180}>
              <Link href={`/buy/ahmedabad/${property.localitySlug}/`} className="group relative block h-full min-h-[200px] overflow-hidden bg-night">
                ...
                <span className="absolute inset-0 flex items-end p-5 text-sm font-semibold text-cream">See the neighbourhood <ArrowUpRight size={15} className="ml-1.5 text-ember transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></span>
              </Link>
            </Reveal>
          </div>
        </div>
        ...
        <div className="grid gap-14 lg:grid-cols-[1fr_360px]">
          <div>
            <div className="flex items-start justify-between gap-6">
              ...
              <button onClick={onSave} aria-pressed={saved} aria-label={saved ? "Remove from saved" : "Save listing"}
            ...
            <div className="mt-12">
              ...
              <p className="stamp mt-3 !text-[9px] text-ink/60">Location shown at locality precision until you connect with the partner — exact address stays private.</p>
            </div>
          </div>
          ...
          <aside className="h-fit border border-ink/15 bg-sand/70 p-7 md:p-8 lg:sticky lg:top-[102px]">
            ...
            <p className="stamp mt-4 text-center !text-[10px] text-ink/60">Usually replies within 4 working hours</p>
          </aside>
        </div>
