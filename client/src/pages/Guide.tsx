...
const notes = [
  { img: "/images/brick-arch.jpg", tag: "Methodology", title: "How we verify a listing against Gujarat RERA", time: "6 min read" },
  { img: "/images/locality-street.jpg", tag: "Locality study", title: "Paldi: reading a neighbourhood by its trees", time: "8 min read" },
  { img: "/images/stepwell.jpg", tag: "Essay", title: "What Adalaj teaches us about trust in layers", time: "5 min read" },
];
        ...
        <div className="container">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {notes.map((n, i) => (
              <Reveal key={n.title} delay={i * 90}>
                <article className="group cursor-pointer border border-ink/12 bg-card motion-lift hover:editorial-shadow">
                  <div className="img-hover aspect-[1.35] bg-sand"><img src={n.img} alt="" className="h-full w-full object-cover" loading="lazy" /></div>
                  <div className="p-6">
          ...
          <Reveal delay={200}>
            <div className="mt-14 flex flex-col items-start justify-between gap-6 border border-ink/12 bg-card p-8 md:flex-row md:items-center md:p-10">
              ...
              <Link href="/search" className="btn-sweep motion-press inline-flex items-center gap-2 bg-brick px-7 py-4 stamp !text-[12px] font-semibold text-paper">Start exploring <ArrowUpRight size={15} /></Link>
            </div>
          </Reveal>
        </div>
      </section>
