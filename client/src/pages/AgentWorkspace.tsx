    ...
    <div className="desk-filter border border-ink/12 bg-card p-4 md:p-5" data-filter-state={active ? "active" : "gated"}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          {fields.map((field) => (
            <button key={field} type="button" disabled={!active} title={active ? undefined : gateNote ?? "This filter activates when a live source feeds the table"} className={`touch-44 inline-flex items-center gap-2 border px-3 py-2 stamp !text-[10px] font-semibold ${active ? "border-ink/12 text-ink/65 hover:border-brick/50 hover:text-brick" : "cursor-not-allowed border-ink/10 ink-4"}`}>
              <ListFilter size={12} />{field}
            </button>
          ))}
        </div>
        <button type="button" disabled={!active || !value} onClick={active ? () => onValueChange("") : undefined} className={`touch-44 stamp !text-[10px] font-semibold underline underline-offset-4 ${active && value ? "text-brick" : "cursor-not-allowed ink-4 no-underline"}`}>
          Clear
        </button>
      </div>
...
function Inquiry({ leads, leadsState }: { leads: LeadRecord[]; leadsState: "loading" | "ready" | "unavailable" }) {
  ...
  return <>
    ...
    <div className="mt-5 flex flex-wrap gap-2">
      {tabs.map((item) => (
        <button key={item.id} type="button" onClick={() => setTab(item.id)} aria-pressed={tab === item.id} className={`border px-4 py-3 stamp !text-[10px] font-semibold transition ${tab === item.id ? "border-brick bg-brick text-cream" : "border-ink/12 bg-card ink-3 hover:border-brick/40"}`}>
