...
pnpm test:report    # the same run, plus tmp/unit.json for test:assert-parity
pnpm test:assert-parity  # fail if the search parity matrix skipped itself
...
- `src/lib/search/sql-page-integration.test.ts` (48 cases) proves the SQL page
  path returns exactly what the JS path returns. It needs a real Postgres and is
  opt-in, so locally:

  ```bash
  pnpm db:setup:sandbox                     # disposable cluster, migrations + seed
  ARCHITECH_PARITY_DATABASE_URL='postgresql://architech:architech@localhost:5432/architech?schema=public' \
    pnpm test:report && pnpm test:assert-parity
  ```

  CI does this on every build against a `postgis/postgis` service. Without the
  variable the file skips and `pnpm test` still reports green — so run
  `test:assert-parity`, not `test`, when you need to know it executed.
