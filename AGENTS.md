...
## Skill selection — do this FIRST, every task

`.agents/skills/` vendors ~460 agent skills. **Before implementing any non-trivial task,
select the best-matching skill and use it. Do not start work with an unselected skill set.**

1. Open **`.agents/skills/INDEX.md`** (generated by `pnpm skills:index`).
2. Start from its **Daily shortlist** — skills already matched to this repo's stack.
   If nothing there fits, search the full tables: `grep -i "<keyword>" .agents/skills/INDEX.md`.
3. Pick the **single best-matching skill**, then read that skill's `SKILL.md` **in full**
   before acting. A skill you have not read is a skill you have not selected.
4. **State your choice** in the reply: "Using skill: `<name>` — because …". If no skill
   matches, say "no skill matched" and proceed. Never skip this step silently.

Tie-breaking: prefer the more specific skill over the more general one; prefer a skill
from the Daily shortlist over an unvendored-nested duplicate; workflow skills (planning,
TDD, debugging, verification) stack on top of stack skills — they do not compete.

After adding, removing, or renaming any skill under `.agents/skills/`, run
`pnpm skills:index` and commit the regenerated `INDEX.md` (same discipline as
`pnpm docs:index`). `pnpm skills:index:check` fails when the index is stale.
