    ...
    <section className="border-y border-ink/12 bg-sand/60 py-20 md:py-28" aria-labelledby="market-directory-title">
      <div className="container">
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
          ...
          <Link href="/buy/" className="link-rail stamp-sm self-start font-semibold text-brick md:self-end">Explore all cities and localities <ArrowUpRight size={14} className="ml-1 inline" /></Link>
        </div>
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[1.4fr_0.6fr]">
          <div>
            <div className="flex items-end justify-between border-b border-ink/15 pb-4"><div><p className="kicker text-brick">Featured project rail</p><h3 className="mt-2 font-display text-3xl font-medium tracking-[-0.03em]">Projects worth a second look.</h3></div><span className="stamp text-ink/55">curated · demo inventory</span></div>
            <div className="grid gap-4 pt-5 md:grid-cols-3">
              {projects.map((project, index) => (
                <Link key={project.name} href={project.href} className="group border border-ink/15 bg-paper p-5 transition-transform duration-300 hover:-translate-y-1 hover:border-brick">
                  <p className="stamp text-brick">Nº {String(index + 1).padStart(2, "0")} · {project.label}</p>
          ...
          <aside className="border-l border-ink/15 pl-6 lg:pl-8">
            ...
            <Link href="/developers/" className="link-rail mt-5 inline-flex items-center gap-1.5 stamp-sm font-semibold text-brick">Browse the developer index <ArrowUpRight size={13} /></Link>
          </aside>
        </div>
        ...
        <div className="mt-16 grid gap-8 border-t border-ink/15 pt-8 lg:grid-cols-[1fr_auto] lg:items-start">
          ...
          <div className="flex flex-col items-start gap-3 lg:items-end"><Link href="/investment/" className="inline-flex items-center gap-2 stamp-sm font-semibold text-brick"><Newspaper size={14} /> India investment lens <ArrowUpRight size={13} /></Link><Link href="/guide/" className="inline-flex items-center gap-2 stamp-sm font-semibold text-ink/65 hover:text-brick">Buying guides and field notes <ArrowUpRight size={13} /></Link></div>
        </div>
