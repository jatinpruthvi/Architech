  ...
  return (
    <div role="status" aria-label="Loading broker workspace" className="space-y-5">
      ...
      <div aria-hidden="true" className="space-y-5">
        <div className="h-7 w-52 animate-pulse rounded-lg bg-[#dce7f3] motion-reduce:animate-none" />
        ...
        <div className="space-y-3 rounded-2xl border border-[var(--tp-border)] bg-white p-4">
          {[0, 1, 2, 3].map((item) => (
            <div key={item} className="h-16 animate-pulse rounded-xl bg-[#edf3f9] motion-reduce:animate-none" />
          ))}
        </div>
      </div>
    </div>
