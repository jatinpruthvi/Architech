    ...
    <StorySurface lang={lang}>
      <div className="max-w-4xl space-y-10">
        <section className="border border-dashed border-ink/25 p-10 text-center">
          ...
          <Link href="/search" className="btn-sweep motion-press mt-8 inline-flex items-center gap-2 bg-brick px-7 py-4 stamp font-semibold text-cream">{isHi ? "घर खोजें" : "Find a home"} <ArrowUpRight size={15} /></Link>
        </section>
        ...
        <section className="border border-ink/12 bg-card p-7">
          ...
          <div className="mt-5 flex flex-wrap gap-2" role="group" aria-label={isHi ? "फ़िल्टर" : "Filters"}>
            {[isHi ? "2 BHK" : "2 BHK", isHi ? "₹1.5 करोड़ से कम" : "Under ₹1.5 Cr", isHi ? "RERA सत्यापित" : "RERA verified"].map((label, index) => (
              <button key={label} aria-pressed={index === 1} className={`touch-44 px-4 py-2.5 stamp-sm font-semibold ${index === 1 ? "bg-brick text-cream" : "border border-ink/20 text-ink/70"}`}>{label}</button>
            ))}
          </div>
          <p className="mt-5 flex items-center gap-2 stamp-sm text-ink/60"><SlidersHorizontal size={12} /> {isHi ? "Storybook में Hindi state preview." : "Storybook previews interaction states before backend data lands."}</p>
        </section>
      </div>
    </StorySurface>
  );
