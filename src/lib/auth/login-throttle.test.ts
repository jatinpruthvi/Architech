...
describe("sign-in throttle", () => {
  ...
  it("the email bucket map stays bounded under a sprayed address space", () => {
  ...
  it("the ip bucket map stays bounded under rotating addresses", () => {
    for (let attempt = 0; attempt < MAX_LOGIN_THROTTLE_BUCKETS + 2000; attempt += 1) {
  ...
  it("expired windows are reclaimed instead of retained forever", () => {
    const start = 1_000_000;
  ...
  it("pruning never weakens the per-account or per-address budget", () => {
    for (let attempt = 0; attempt < MAX_ATTEMPTS_PER_EMAIL; attempt += 1) {
