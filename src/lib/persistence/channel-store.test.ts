...
describe("saveChannelDealSplitForServer (Prisma path) — commission amount validation", () => {
  ...
  /* BUG-2026-001 closed the negative and fractional holes but not
  ...
  it("rejects a commission past the BIGINT column range with 400", async () => {
    const result = await saveChannelDealSplitForServer(
      "deal-1",
  ...
  it("any commission that does reach the write fits BIGINT", async () => {
