      ...
      <div className="bg-paper pt-[78px] text-ink">
        <section className="container py-24">
          <EmptyState
            ...
            action={<Link href="/search" className="clay-fill btn-sweep motion-press inline-flex items-center gap-2 bg-brick px-8 py-5 stamp font-semibold text-cream">{t.saved.emptyCta} <ArrowUpRight size={15} /></Link>}
          />
        </section>
      </div>
    );
    ...
    <div className="bg-paper pt-[78px] text-ink">
      <section className="border-b border-ink/12 bg-sand/70 py-14 md:py-20">
        <div className="container">
          ...
          {t.common.translationNote && <p className="stamp-sm mt-4 max-w-[520px] text-ink/55">{t.common.translationNote}</p>}
        </div>
      </section>
      <section className="container py-14 md:py-20">
        ...
        <div className="mt-12 flex items-center justify-between border-t border-ink/10 pt-6">
          <p className="stamp-sm text-ink/60">{t.saved.tip}</p>
          <Link href="/search" className="group inline-flex items-center gap-2 stamp font-semibold text-brick">{t.saved.keepExploring} <ArrowUpRight size={15} className="transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></Link>
        </div>
      </section>
    </div>
  );
