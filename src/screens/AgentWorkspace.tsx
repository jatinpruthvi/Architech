...
import { demandByListing, draftPortfolio, firstResponseStats, leadFunnel } from "@/lib/broker/analytics";
...
/** Live org-scoped lead inbox state. `unavailable` = the API answered 403/5xx
    or failed outright — the desk must show "no live source", not a confident
    zero (a hardcoded 0 is a lie of omission; that was finding I-6). */
function useBrokerDeskLeads(): { state: "loading" | "ready" | "unavailable"; leads: LeadRecord[] } {
  ...
  useEffect(() => {
    ...
    fetch("/api/broker/leads", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!cancelled) setValue({ state: "ready", leads: Array.isArray(payload.leads) ? payload.leads : [] });
      })
      .catch(() => { if (!cancelled) setValue({ state: "unavailable", leads: [] }); });
    return () => { cancelled = true; };
  }, []);
  return value;
}

/** A KPI value that only renders a number when it came off a live read. */
function liveCount(state: "loading" | "ready" | "unavailable", value: number | null): string {
  if (state !== "ready" || value === null) return "—";
  return String(value);
}
...
function EmptyState({ title, body, action, href }: { title: string; body: string; action?: string; href?: string }) {
  return <div role="status" aria-live="polite" className="empty-state desk-empty border border-dashed border-ink/20 bg-sand/45 p-8 md:p-10"><div className="flex items-start gap-4"><span className="empty-state-icon grid h-11 w-11 shrink-0 place-items-center rounded-t-full bg-brick/10 text-brick"><Archive size={19} /></span><div><p className="stamp text-brick/70">No live records · source gate</p><h3 className="mt-2 font-display text-2xl font-medium tracking-[-0.02em]">{title}</h3><p className="mt-2 max-w-xl text-sm leading-6 text-ink/60">{body}</p>{action && href ? <Link href={href} className="mt-5 inline-flex items-center gap-2 stamp-sm font-semibold text-brick">{action}<ArrowUpRight size={13} /></Link> : null}</div></div></div>;
}

/* Two honest modes, dead inputs are not one of them (I-6):
 *  - ACTIVE: a query string is controlled by the caller and actually filters
 *    a live row set (My listings · lead inbox · inquiry ledger).
 *  - GATED: no source feeds the surface yet, so the strip is fully disabled
 *    and says WHY — the filter-bar doctrine ("disabled + visible, never
 *    pretend-active") applied to a partner desk. */
function FilterStrip({ searchLabel = "Search records", fields = ["Listing type", "Property type", "Locality", "Budget", "Area", "Bedrooms"], value, onValueChange, gateNote }: { searchLabel?: string; fields?: string[]; value?: string; onValueChange?: (value: string) => void; gateNote?: string }) {
  const active = typeof onValueChange === "function";
  return (
    <div className="desk-filter border border-ink/12 bg-card p-4 md:p-5" data-filter-state={active ? "active" : "gated"}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <label className={`field-shell flex min-h-11 flex-1 items-center gap-2 border px-3 text-sm focus-within:border-brick ${active ? "border-ink/12 bg-paper text-ink/55" : "border-ink/8 bg-sand/40 ink-4"}`}>
          <Search size={15} />
          <span className="sr-only">{searchLabel}</span>
          <input
            aria-label={searchLabel}
            placeholder={active ? searchLabel : `${searchLabel} · activates with a live source`}
            className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-ink/40"
            value={value ?? ""}
            disabled={!active}
            onChange={active ? (event) => onValueChange(event.target.value) : undefined}
          />
        </label>
        <div className="flex flex-wrap gap-2">
          {fields.map((field) => (
            <button key={field} type="button" disabled={!active} title={active ? undefined : gateNote ?? "This filter activates when a live source feeds the table"} className={`touch-44 inline-flex items-center gap-2 border px-3 py-2 stamp-sm font-semibold ${active ? "border-ink/12 text-ink/65 hover:border-brick/50 hover:text-brick" : "cursor-not-allowed border-ink/10 ink-4"}`}>
              <ListFilter size={12} />{field}
            </button>
          ))}
        </div>
        <button type="button" disabled={!active || !value} onClick={active ? () => onValueChange("") : undefined} className={`touch-44 stamp-sm font-semibold underline underline-offset-4 ${active && value ? "text-brick" : "cursor-not-allowed ink-4 no-underline"}`}>
          Clear
        </button>
      </div>
      {!active && <p className="mt-3 stamp ink-3">{gateNote ?? "Filters stay honest: they activate only when a live source feeds the rows below."}</p>}
    </div>
  );
}
function TableFrame({ columns, children, tone = "default" }: { columns: string[]; children: React.ReactNode; tone?: "default" | "ledger" }) {
  return <div className={`desk-table ${tone === "ledger" ? "desk-table-ledger" : ""} overflow-x-auto border border-ink/12 bg-card`}><table className="min-w-[760px] w-full text-left text-sm"><thead className="border-b border-ink/10 bg-sand/45"><tr>{columns.map((column) => <th key={column} className="px-4 py-3 stamp-sm text-ink/55">{column}</th>)}</tr></thead><tbody>{children}</tbody></table></div>;
}
...
function PerformanceRegister({ leads, leadsState, drafts }: { leads: LeadRecord[]; leadsState: "loading" | "ready" | "unavailable"; drafts: ListingDraft[] }) {
  ...
  const demand = leadsState === "ready" ? demandByListing(leads) : [];
  const portfolio = draftPortfolio(drafts);
  const formatMinutes = (minutes: number) => (minutes < 60 ? `${minutes} min` : minutes < 1440 ? `${Math.round(minutes / 6) / 10} hours` : `${Math.round(minutes / 144) / 10} days`);
  return <section className="mt-12"><DeskRule label="Performance register" detail="FUNNEL / FIRST RESPONSE / DEMAND" />{leadsState === "ready" ? <div className="mt-5 grid gap-4 lg:grid-cols-3">
    <article className="border border-ink/12 bg-card p-6"><p className="stamp ink-3">Enquiry funnel</p><p className="mt-6 index-num text-6xl">{funnel.total}</p><p className="mt-2 text-sm ink-2">active leads in the funnel</p><div className="mt-6 space-y-2 border-t border-ink/10 pt-4 text-sm"><p className="flex justify-between"><span className="ink-2">Fresh</span><span className="font-mono">{funnel.fresh}</span></p><p className="flex justify-between"><span className="ink-2">Acknowledged</span><span className="font-mono">{funnel.acknowledged}</span></p><p className="flex justify-between"><span className="ink-2">Replied</span><span className="font-mono">{funnel.replied}</span></p><p className="flex justify-between"><span className="ink-2">Closed</span><span className="font-mono">{funnel.closed}</span></p></div>{funnel.excluded > 0 ? <p className="mt-4 stamp ink-4">+{funnel.excluded} consent-revoked or removed, excluded from the funnel</p> : null}</article>
    <article className="border border-ink/12 bg-card p-6"><p className="stamp ink-3">First response</p>{response ? <><p className="mt-6 index-num text-6xl">{formatMinutes(response.medianMinutes)}</p><p className="mt-2 text-sm ink-2">median, enquiry to first acknowledgment or reply</p><p className="mt-6 border-t border-ink/10 pt-4 stamp ink-4">sample: {response.count} responded lead{response.count === 1 ? "" : "s"}</p></> : <><p className="mt-6 index-num text-6xl">—</p><p className="mt-2 text-sm ink-2">Appears once the first lead is acknowledged or replied to — a median without a response would invent one.</p></>}</article>
    <article className="border border-ink/12 bg-card p-6"><p className="stamp ink-3">Demand by listing</p>{demand.length ? <div className="mt-5 space-y-3">{demand.map((row) => <p key={row.listingTitle} className="flex items-baseline justify-between gap-3 text-sm"><span className="min-w-0 truncate ink-2">{row.listingTitle}</span><span className="font-mono">{row.leads}</span></p>)}</div> : <p className="mt-6 max-w-[18rem] text-sm leading-6 ink-2">No attributed enquiries yet — this ranks only listings that have received a lead.</p>}<div className="mt-6 border-t border-ink/10 pt-4"><p className="stamp ink-3">Draft portfolio</p><p className="mt-3 flex flex-wrap gap-2">{portfolio.byStatus.map((row) => <span key={row.status} className="stamp ink-3 bg-sand px-2 py-1">{row.status} · {row.count}</span>)}</p>{portfolio.lastEditDays === null ? null : <p className="mt-3 text-sm ink-2">Last draft touched {portfolio.lastEditDays === 0 ? "today" : `${portfolio.lastEditDays} day${portfolio.lastEditDays === 1 ? "" : "s"} ago`}.</p>}</div></article>
  </div> : <div className="mt-5 border border-ink/12 bg-card p-6"><p className="font-display text-xl font-medium">Analytics gather when the lead inbox connects.</p><p className="mt-2 max-w-xl text-sm leading-6 ink-2">Funnel, first-response time, and demand are computed from your real inbox and drafts only — they are left blank rather than projected from nothing.</p></div>}</section>;
}

function Dashboard({ drafts, leads, leadsState }: { drafts: ListingDraft[]; leads: LeadRecord[]; leadsState: "loading" | "ready" | "unavailable" }) {
  /* Numbers only where a live read returned them. "—" with an explanatory
     sublabel, not a confident 0, wherever no source feeds the metric — the
     old hardcoded zeros were I-6's exact complaint. */
  const kpis = [
    [Inbox, "Total inquiry", liveCount(leadsState, leads.length), leadsState === "ready" ? "Live from your lead inbox" : "Awaiting a live lead source"],
    [PhoneCall, "Callback queue", liveCount(leadsState, leads.filter((lead) => lead.status === "ACKNOWLEDGED").length), leadsState === "ready" ? "Acknowledged, awaiting your reply" : "No live source"],
    [BriefcaseBusiness, "My properties", String(drafts.length), "Drafts and reviewed submissions"],
    [CheckCircle2, "Deals closed", liveCount(leadsState, leads.filter((lead) => lead.status === "CLOSED").length), leadsState === "ready" ? "Leads closed in the inbox" : "No outcome data recorded"],
    [CalendarClock, "Scheduled visits", "—", "No visit scheduler is connected yet"],
  ] as const;
...
function Inquiry({ leads, leadsState }: { leads: LeadRecord[]; leadsState: "loading" | "ready" | "unavailable" }) {
  const [tab, setTab] = useState<"fresh" | "follow" | "all">("fresh");
  const [query, setQuery] = useState("");
  const buckets: Record<"fresh" | "follow" | "all", LeadStatus[]> = {
    fresh: ["NEW"],
    follow: ["ACKNOWLEDGED", "REPLIED"],
    all: ["NEW", "ACKNOWLEDGED", "REPLIED", "CLOSED"],
  };
  const scoped = leads.filter((lead) => buckets[tab].includes(lead.status));
  const needle = query.trim().toLowerCase();
  const visible = needle ? scoped.filter((lead) => [lead.id, lead.name, lead.phoneMasked, lead.listingTitle, lead.message].filter(Boolean).join(" ").toLowerCase().includes(needle)) : scoped;
  const tabs: Array<{ id: "fresh" | "follow" | "all"; label: string; count: number }> = [
    { id: "fresh", label: "Fresh inquiry", count: leads.filter((lead) => lead.status === "NEW").length },
    { id: "follow", label: "My follow-up", count: leads.filter((lead) => lead.status === "ACKNOWLEDGED" || lead.status === "REPLIED").length },
    { id: "all", label: "All leads", count: leads.length },
  ];
  return <>
    <LedgerIntro label="Buyer intent" detail="LEDGER / 02" body="Fresh inquiry, follow-up, and all-lead states share one reviewable source trail. Contact access stays behind consent." />
    <div className="mt-7"><FilterStrip searchLabel="Contact number, name, or lead ID" fields={["Listing type", "Property type", "Locality", "Budget", "Area", "Bedrooms", "Lead source", "Inquiry date"]} value={query} onValueChange={leadsState === "ready" ? setQuery : undefined} gateNote="Filters activate the moment your lead inbox is connected." /></div>
    <div className="mt-5 flex flex-wrap gap-2">
      {tabs.map((item) => (
        <button key={item.id} type="button" onClick={() => setTab(item.id)} aria-pressed={tab === item.id} className={`border px-4 py-3 stamp-sm font-semibold transition ${tab === item.id ? "border-brick bg-brick text-cream" : "border-ink/12 bg-card ink-3 hover:border-brick/40"}`}>
          {item.label} ({leadsState === "ready" ? item.count : "—"})
        </button>
      ))}
    </div>
    <div className="mt-5">
      <TableFrame tone="ledger" columns={["Lead ID", "Type", "Listing", "Contact", "Message", "Source", "Inquiry date"]}>
        {visible.length ? visible.map((lead) => (
          <tr key={lead.id} className="border-t border-ink/10">
            <td className="px-4 py-4 font-mono text-xs">{lead.id}</td>
            <td className="px-4 py-4">{lead.status.toLowerCase()}</td>
            <td className="px-4 py-4">{lead.listingTitle || "—"}</td>
            <td className="px-4 py-4">{lead.name} · {lead.phoneMasked}</td>
            <td className="max-w-48 truncate px-4 py-4 ink-2">{lead.message || "—"}</td>
            <td className="px-4 py-4 stamp ink-3">{lead.mode === "DIRECT_CONSENTED" ? "consented" : "masked"}</td>
            <td className="px-4 py-4">{lead.createdAt ? new Date(lead.createdAt).toISOString().slice(0, 10) : "—"}</td>
          </tr>
        )) : <tr><td colSpan={7} className="px-4 py-12 text-center"><EmptyState title={needle ? "No inquiries match that search" : "No inquiries yet"} body={leadsState === "unavailable" ? "Your organization lead inbox is not reachable for this account — once connected, fresh, follow-up, and all-lead states appear here with consent, source, and status history." : "Fresh, follow-up, and all-lead states will appear here with consent, source, and status history."} action={needle ? undefined : "Create a requirement"} href={needle ? undefined : "/requirements"} /></td></tr>}
      </TableFrame>
    </div>
  </>;
}

function Leads({ leads, leadsState }: { leads: LeadRecord[]; leadsState: "loading" | "ready" | "unavailable" }) {
  const [query, setQuery] = useState("");
  const open = leads.filter((lead) => lead.status === "NEW" || lead.status === "ACKNOWLEDGED" || lead.status === "REPLIED");
  const needle = query.trim().toLowerCase();
  const visible = needle ? open.filter((lead) => [lead.id, lead.name, lead.phoneMasked, lead.listingTitle, lead.message].filter(Boolean).join(" ").toLowerCase().includes(needle)) : open;
  return <>
    <LedgerIntro label="Open demand" detail="LEDGER / 04" body="A lead is not just a row. It is a consented buyer signal with a status, source, freshness, and next action." />
    <div className="mt-7"><FilterStrip searchLabel="Contact number, name, or inquiry by" fields={["Listing type", "Property type", "Locality", "Budget", "Area", "Bedrooms", "Inquiry date"]} value={query} onValueChange={leadsState === "ready" ? setQuery : undefined} gateNote="Filters activate the moment your lead inbox is connected." /></div>
    <div className="mt-5 flex items-center justify-between border-b border-ink/12 pb-4">
      <p className="font-display text-2xl">Open leads <span className="text-brick">{leadsState === "ready" ? open.length : "—"}</span></p>
      <span className="stamp text-ink/45">SOURCE + CONSENT REQUIRED</span>
    </div>
    <div className="mt-5">
      <TableFrame tone="ledger" columns={["Status", "Lead ID", "Listing", "Contact", "Message", "Inquiry date"]}>
        {visible.length ? visible.map((lead) => (
          <tr key={lead.id} className="border-t border-ink/10">
            <td className="px-4 py-4"><span className="stamp ink-3 bg-sand px-2 py-1">{lead.status.toLowerCase()}</span></td>
            <td className="px-4 py-4 font-mono text-xs">{lead.id}</td>
            <td className="px-4 py-4">{lead.listingTitle || "—"}</td>
            <td className="px-4 py-4">{lead.name} · {lead.phoneMasked}</td>
            <td className="max-w-56 truncate px-4 py-4 ink-2">{lead.message || "—"}</td>
            <td className="px-4 py-4">{lead.createdAt ? new Date(lead.createdAt).toISOString().slice(0, 10) : "—"}</td>
          </tr>
        )) : <tr><td colSpan={6} className="px-4 py-12 text-center"><EmptyState title={needle ? "No open leads match that search" : "No leads yet"} body={leadsState === "unavailable" ? "The lead inbox could not be reached for this account. Once connected, leads appear here with status, consent, source, and contact access history." : "Leads will appear with status, consent, source, and contact access history after a live lead source is connected."} /></td></tr>}
      </TableFrame>
    </div>
  </>;
}
...
export default function AgentWorkspace({ section = "dashboard" }: { section?: AgentSection }) {
  ...
  const body = useMemo(() => {
    if (section === "dashboard") return <Dashboard drafts={drafts} leads={leads} leadsState={leadsState} />;
    if (section === "inquiry") return <Inquiry leads={leads} leadsState={leadsState} />;
    ...
    return <Profile />;
  }, [drafts, leads, leadsState, section]);
