...
import { acceptChannelMatchForServer } from "@/lib/persistence/channel-store";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await acceptChannelMatchForServer(decodeURIComponent(id), access.session);
