...
import { rejectChannelMatchForServer } from "@/lib/persistence/channel-store";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await rejectChannelMatchForServer(decodeURIComponent(id), access.session);
